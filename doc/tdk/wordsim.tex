...
\begin{figure}[H]
	...
	\caption{Unrolling the LSTM network over 3 time steps}
\end{figure}
...
The sequence to sequence model (seq2seq) was first introduced by \cite{Cho:2014}, but they only used it to re-rank sentences instead of generating completely new ones, which was first done by \cite{Sutskever:2014}.

\begin{figure}[H]
	\label{fig:232}
	\centering
	\includegraphics[width=0.9\textwidth]{pics/seq2seq.png}
	\caption{LSTM based Seq2seq model used to generate a reply from a source sentence}
\end{figure}
